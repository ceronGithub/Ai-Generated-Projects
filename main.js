...
let uploadedFiles = []; // Shared array
let capturedResults = []; // Shared array
let speed = 0.5;
...
function animateStars() {
    sCtx.fillStyle = document.documentElement.getAttribute('data-theme') === 'light' ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.2)';
    ...
    stars.forEach(s => {
        ...
        sCtx.beginPath(); sCtx.arc(x, y, (1 - s.z / starCanvas.width) * 3, 0, Math.PI * 2);
        sCtx.fillStyle = document.documentElement.getAttribute('data-theme') === 'light' ? `rgba(0,0,0,${s.o})` : `rgba(255,255,255,${s.o})`;
        sCtx.fill();
    });

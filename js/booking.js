...
(function () {
  ...
  inp.guestName.addEventListener("input", function () {
    if (this.value.includes("@")) {
      ...
      setTimeout(() => {
        this.placeholder = "Full name of guest";
      }, 3000);
    }
  });
    ...
    if (_forcedCheckinMins !== null) {
      ...
      if (forcedHour >= 12) {
        ...
        if (selectedTour === "Day tour") {
          ...
          selectedHrs = 0;
          document
            .querySelectorAll(".tour-btn[data-hrs]")
  ...
  document.querySelectorAll(".tour-btn[data-tour]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document
        .querySelectorAll(".tour-btn[data-tour]")
        .forEach((b) => b.classList.remove("active"));
      ...
      const autoHrs =
        selectedTour === "Day tour" || selectedTour === "Night tour" ? 10 : 21;
      selectedHrs = autoHrs;
  ...
  document.querySelectorAll(".tour-btn[data-mop]").forEach((btn) => {
    btn.addEventListener("click", () => {
      document
        .querySelectorAll(".tour-btn[data-mop]")
        .forEach((b) => b.classList.remove("active"));
      btn.classList.add("active");
  ...
  inp.extraTimeExt.addEventListener("input", () => {
    updateCalculations();
    updateCheckout();
  });
  ...
  function showAutoCheckinNotice(msg) {
    if (!_autoCheckinNotice) {
      ...
      inp.checkinTime.parentNode.insertBefore(
        _autoCheckinNotice,
        inp.checkinTime.nextSibling,
      );
    }
    if (msg) {
  ...
  async function autoSetCheckinFromExistingBooking(dateStr) {
    ...
    try {
      ...
      if (allBookings.length === 0) {
        ...
        updateCheckout();
        showAutoCheckinNotice(
          "✅ No existing bookings on this date — select a check-in time",
        );
      ...
      const countLabel =
        bookingCount === 1
          ? "1 existing booking"
          : `${bookingCount} existing bookings`;
      showAutoCheckinNotice(
      ...
      showAutoCheckinNotice(
        "⚠️ Could not reach database — please set check-in time manually",
      );
    }
  }
  ...
  function updateCheckout() {
    ...
    let label =
      selectedTour === "3D 2N" ? "21 hrs + 21 hrs" : `${selectedHrs} hrs`;
    if (extraTimeExt > 0)
      label += ` + ${extraTimeExt} hr${extraTimeExt !== 1 ? "s" : ""} extension`;
  ...
  btnProceed.addEventListener("click", () => {
    ...
    const down =
      "\u20b1" +
      Number(inp.downPayment.value).toLocaleString("en-PH", {
        minimumFractionDigits: 2,
      });
    const balance =
      "\u20b1" +
      Number(inp.balance.value).toLocaleString("en-PH", {
        minimumFractionDigits: 2,
      });
    ...
    const message = `Good day ${guestFirstName},
...
+63 954 184 3179`;
    ...
    window.scrollTo(0, 0);
    setTimeout(() => {
      if (typeof window.refreshEmailPreview === "function")
        window.refreshEmailPreview();
    }, 100);
  });
